...
- [ ] **步骤 3：编写 `pyproject.toml`, `requirements.txt`, `opspilot/__init__.py` 与 `opspilot/config.py`**
...
- [ ] **步骤 3：编写 `opspilot/schemas/task.py` 与 `opspilot/schemas/alert.py`**
...
- [ ] **步骤 1：编写 Webhook 路由接收与任务分发的失败测试**
...
- [ ] **步骤 3：编写 `opspilot/api/routes_webhook.py` 并挂载到 `main.py`**
...
- [ ] **步骤 3：编写 `opspilot/api/routes_diagnose.py` 并挂载**
